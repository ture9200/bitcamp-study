...
public class Exam0210 {
  ...
  // 인스턴스 주소를 받을 생성자를 만들 필요 없다.
  static class A {} 

  // => 다른 스태틱 멤버 (static nested class 포함) 
  ...
  class B {} // static 을 붙이는 게 기본 , 언제 안붙여야하냐? 
  ...
  public static void main(String[] args) {
    ...
    // => 문법
    //      new 수퍼클래스() {수퍼 클래스를 상속 받은 익명 서브 클래스 정의} => 수퍼클래스의 생성자 호출 
    ...
    //      new 인터페이스1, 인터페이스2() {인터페이스를 구현한 익명 클래스 정의} => 인터페이스는 무조건 하나라서 이런 문법은 없다. 

    Object obj = new Object() { 
      // 인터페이스 명이나 클래스명을 타입을 해야하는데 익명클래스는 
      // 클래스이름이 없기 때문에 수퍼클래스나 인터페이스를 구현하는 인터페이스 명이 와야한다. 
