...
// JVM 을 실행하는 동안 한 번 클래스가 로딩되면 같은 클래스에 대해 중복 로딩되지 않는다. 
// 클래스 변수는 클래스가 로딩될 때 생성되기 때문에 딱 한 번 생성된다. 
//
...
//      - 주의! 
//        바이트코드를 그대로 메모리에 두는 것이 아니라, 멤버에 따라 적절하게 분류한다. 
