...
public class Client0210 {
  public static void main(String[] args) {
    ...
    while (true) {
      System.out.print("이름? ");
      String name = keyScan.nextLine(); //이름을 입력받는다. 

      if (name.equalsIgnoreCase("quit")) { // quit이면 프로그램 끝낸다. 
        break;
      }
      ...
      try (Socket socket = new Socket("localhost", 8888);
        ...
        if (name.equals("stop")) // stop하면 반복하는걸 끝낸다. 
          break; 

        // quit하면 서버에 데이터를 보내지도 않고 stop하면 서버에 데이터를 보내고 응답받고 끝낸다 
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
