...
public class CalcServer {
  ...
  static class RequestHandler extends Thread {
    ...
    public RequestHandler(Socket socket) { // 생성자에 소켓 정보가 넘어오면 보관하고 
      this.socket = socket; 
    }
    ...
    public void run() {
      ...
      // 이 메서드에 둔다. 
      try (Socket socket2 = socket; // close()를 자동으로 호출 
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      Socket socket = ss.accept(); // 클라이언트 요청을 받는다. 
      InetSocketAddress remoteAddr = (InetSocketAddress) socket.getRemoteSocketAddress();
      // socketAddress를 상속해서 만든 서브클래스인 InetSocketAddress 객체를 리턴, type캐스팅 
      // 인터넷에 연결되었을때는 InetSocketAddress 다 
      System.out.printf("클라이언트(%s:%d)가 연결되었음!\n", //
          remoteAddr.getAddress(), remoteAddr.getPort()); 
      // getAddress() => ip address를 알아내는 메서드 ,특정 범위 밖의 아이피 주소가 접속했을 때 접속 거절가능 
      // getPort() => port번호를 알아내는 메서드 

      // 독립적으로 수행할 코드를 갖고 있는 스레드 객체를 생성한다.
      // 클라이언트가 접속하는 순간 스레드를 생성해서 별도 실행흐름을 만들고 
      RequestHandler requestHandler = new RequestHandler(socket);

      // 그리고 작업을 실행시킨다.
      // => 스레드를 실행시킨 후 바로 리턴한다.
      requestHandler.start(); //별도로 실행을 시작한다. 
