...
// Spring 프레임워크 의 설정 정보를 담은 클래스 
//
//1) 스프링 IOC 컨테이너에게 객체를 생성하기 위해 패키지를 알려준다. 
//=> 스프링 IOC 컨테이너는 패키지를 뒤져서 @Component, @Controller,@RestController, @Service등이 
//   붙은 클래스를 찾아내어 객체를 생성하고 보관한다. 
@ComponentScan("com.eomcs.mylist")

//2) DB 관련 설정을 할때 DBMS 정보를 담은 PROPERTIES 파일을 로딩한다. 
@PropertySource("classpath:com/eomcs/mylist/conf/jdbc.properties")

//3) DAO 인터페이스 구현체를 자동생성
@MapperScan("com.eomcs.mylist.dao")
public class AppConfig {
  
  
  @Bean //SpringIOC 컨테이너에게 다음 메서드를 실행한 후 리턴된 객체를 보관하라는 명령이다. 
